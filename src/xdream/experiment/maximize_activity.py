...
from xdream.core.experiment                import ZdreamExperiment
from xdream.core.generator                 import Generator, DeePSiMGenerator
from xdream.core.optimizer                 import CMAESOptimizer, GeneticOptimizer, HybridOptimizer, Optimizer
from xdream.core.scorer                    import ActivityScorer, Scorer
from xdream.core.subject                   import InSilicoSubject, TorchNetworkSubject
from xdream.core.utils.dataset             import ExperimentDataset, MiniImageNet, NaturalStimuliLoader, RandomImageDataset
from xdream.core.utils.logger              import DisplayScreen, Logger, LoguruLogger
from xdream.core.utils.message             import ZdreamMessage
from xdream.core.utils.misc                import concatenate_images, device
from xdream.core.utils.parameters          import ArgParams, ParamConfig
from xdream.core.utils.probe               import RecordingProbe
from xdream.core.utils.types               import Codes, ScoringUnits, Stimuli, Fitness, States
...
from xdream.experiment.utils.args            import ExperimentArgParams
from xdream.experiment.utils.parsing         import parse_boolean_string, parse_net_loading, parse_recording, parse_scoring
from xdream.experiment.utils.misc            import BaseZdreamMultiExperiment, make_dir
...
class MaximizeActivityExperiment(ZdreamExperiment):
    ...
    def _from_config(cls, conf : ParamConfig) -> 'MaximizeActivityExperiment':
        ...
        # In the optimizer section, modify the hybrid case:
        elif PARAM_optim_type.startswith('hybrid'):
            n_iter = 500
            
            # Parse the optimizer type (format: "hybrid" or "hybrid:param_set_name")
            if ':' in PARAM_optim_type:
                _, param_set_name = PARAM_optim_type.split(':', 1)
            else:
                param_set_name = "default"
            
            # Define parameter sets
            HYBRID_PARAMETER_SETS = {
                "set1": {
                    "inertia_max": 0.99,
                    "inertia_min": 0.95,
                    "cognitive": 2.0,
            ...
            param_dict = HYBRID_PARAMETER_SETS.get(param_set_name, HYBRID_PARAMETER_SETS["default"])
            
            # Create PSO parameters
            pso_params = {
                'inertia_max': param_dict['inertia_max'],
                'inertia_min': param_dict['inertia_min'],
                'cognitive': param_dict['cognitive'],
                'social': param_dict['social'],
                'v_clip': param_dict['v_clip'],
                'num_informants': param_dict['num_informants'],
            ...
            optim = HybridOptimizer(
                ...
                first_PSO_interval=param_dict['first_PSO_interval'],
                second_PSO_interval=param_dict['second_PSO_interval']
            )
        else:
...
# WORK IN PROGRESS GIACO
class OptimizerVariantMultiExperiment(BaseZdreamMultiExperiment):

    def _init(self):
        
        super()._init()
        
        self._data['desc'   ] = 'Performance comparison between optimizers' # TODO
        
        # Stores the best code and score for each variant #TODO: change according to our aim
        self._data['optim_scores'] = defaultdict(lambda: defaultdict(lambda: {'mean': [], 'best': [], 'elapsed_time': []})) 
    ...
    def _progress(
        ...
        scr_layer = str(conf[ExperimentArgParams.RecordingLayers.value])
        variant   = str(conf[ExperimentArgParams.OptimType      .value])
        
        
        
        self._data['optim_scores'][scr_layer][variant]['mean'].append(msg.stats_gen['mean_gens'])
        self._data['optim_scores'][scr_layer][variant]['best'].append(msg.stats_gen['best_gens'])
        self._data['optim_scores'][scr_layer][variant]['elapsed_time'].append(self.exp_time[-1])
        
        
        
        # Clear message histories that aren't needed
        if len(msg.codes_history) > 1:
            msg.codes_history = [msg.codes_history[-1]]  # Keep only last
        if len(msg.states_history) > 1:
            msg.states_history = [msg.states_history[-1]]  # Keep only last
    ...
    def _finish(self):
        ...
        self._data['optim_scores'] = {
            neuron: {
                variant: {
                    'mean': mean_best_dict['mean'],
                    'best': mean_best_dict['best'],
                    'elapsed_time': mean_best_dict['elapsed_time']
                    
                }
                for variant, mean_best_dict in scores.items()
            }
            for neuron, scores in self._data['optim_scores'].items()
        }
        
        super()._finish()


class OptParamsVariantMultiExperiment(BaseZdreamMultiExperiment):   
    
    def _init(self):
        
        super()._init()
        
        self._data['desc'  ] = 'Finetunes hybrid optimizer parameters' 


        self._data['optim_scores'] = defaultdict(lambda: defaultdict(lambda: {
            'mean_gens': [],    
            'best_gens': [],    
            'final_best': None,    
            'elapsed_time': [],   
    ...
    def _progress(
        ...
        # Parse the optimizer type (format: "hybrid" or "hybrid:param_set_name")
        optim_type_str = str(conf[ExperimentArgParams.OptimType.value])
        
        if ':' in optim_type_str:
            _, param_set_name = optim_type_str.split(':', 1)
        else:
            param_set_name = "default"
        ...
        data_obj = self._data['optim_scores'][scr_layer][param_set_name]
        data_obj['mean_gens'].append(msg.stats_gen['mean_gens'])
        data_obj['best_gens'].append(msg.stats_gen['best_gens'])
        data_obj['elapsed_time'].append(self.exp_time[-1])
        data_obj['params'] = param_dict
    def _finish(self):
        ...
        self._data['optim_scores'] = {
            ...
            } for layer, variants in self._data['optim_scores'].items()
        }
        
        super()._finish()

    
#END WORK IN PROGRESS GIACO
