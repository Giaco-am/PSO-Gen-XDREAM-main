...
from xdream.core.utils.types import RecordingUnits
...
def to_regular_dict(d):
    """Convert a (possibly nested) defaultdict to a regular dict.

    This function recursively converts defaultdict objects into regular dictionaries.
    If the input is a nested dictionary (defaultdict or regular dict), it recursively
    processes all nested dictionaries as well.

    Args:
        d: Input dictionary (defaultdict or regular dict) to be converted

    Returns:
        Regular dictionary with the same structure as input
    """
